...
static binary_tree **find_child_ref(binary_tree *parent, int value)
{
    binary_tree **child_ref = parent->value > value ? &parent->left : &parent->right;
...
{
    binary_tree **node_ref = find_child_ref(tree, value);
    binary_tree *node = *node_ref;
...
bool bt_is_empty(binary_tree *tree)
{
    return tree == BT_EMPTY;
}

void bt_insert(binary_tree **tree_ref, int value)
{
    if (!tree_ref) return;
...
void bt_remove(binary_tree **tree_ref, int value)
{
    if (!tree_ref) return;
...
bool bt_contains(binary_tree *tree, int value)
{
    if (!tree) return false;
    
    if (tree->value == value) return true;
    
    binary_tree **child_ref = find_child_ref(tree, value);
    return *child_ref;
}

void bt_rebalance(binary_tree **tree_ref)
{
    if (!*tree_ref) return;
